...
function client_area_ensure_client_area_ticket_messages_table(): void
{
    $db = client_area_require_db();
...
function client_area_ticket_messages_map(mysqli $db, array $ticketIds): array
{
...
try {
    ...
    client_area_ensure_client_area_ticket_messages_table();
    ...
    if (str_contains($contentType, 'application/json')) {
        ...
        $ticketIds = array_map(static fn (array $row): int => (int) ($row['id'] ?? 0), $ticketRows);
        $messagesMap = client_area_ticket_messages_map($db, $ticketIds);
    ...
    if ($ticketId > 0) {
        ...
        if ($messageStmt) {
            $messageStmt->bind_param('isss', $ticketId, $customerName, $message, $attachmentsJson);
            $messageStmt->execute();
            $messageStmt->close();
        }
    }
