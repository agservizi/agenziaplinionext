...
if (!is_array($file) || (int) ($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
    admin_api_json(['message' => 'Allega un file proposta valido.'], 400);
}
...
$originalName = trim((string) ($file['name'] ?? 'proposta.pdf'));
...
$uploadDir = $publicRoot . '/uploads/web-agency-quotes';
if (!is_dir($uploadDir) && !mkdir($uploadDir, 0775, true) && !is_dir($uploadDir)) {
    admin_api_json(['message' => 'Impossibile creare cartella upload proposte.'], 500);
}

$safeName = preg_replace('/[^a-zA-Z0-9._-]+/', '-', $originalName) ?: 'proposta.' . $extension;
$storedName = sprintf('web-agency-%d-%s-%s', $requestId, date('YmdHis'), $safeName);
$destination = $uploadDir . '/' . $storedName;

if (!move_uploaded_file($tmpPath, $destination)) {
    admin_api_json(['message' => 'Impossibile salvare il file proposta.'], 500);
}

$publicUrl = '/uploads/web-agency-quotes/' . rawurlencode($storedName);
...
if (!$update) {
    admin_api_json(['message' => 'Errore aggiornamento proposta progetto'], 500);
}
...
admin_api_json([
    'ok' => true,
    'message' => 'Proposta allegata e disponibile per il cliente.',
    'quote' => [
        'fileName' => $originalName,
        'url' => $publicUrl,
        'sentAt' => date('c'),
