...
function booking_google_error_message(array $response, string $fallback): string
{
    $message = '';
    ...
    if ($message === '' && isset($response['body']) && is_string($response['body'])) {
...
function booking_is_attendees_permission_error(array $response): bool
{
    $message = strtolower(booking_google_error_message($response, ''));
    if ($message === '') {
        return false;
    }

    return str_contains($message, 'service accounts cannot invite attendees')
        || str_contains($message, 'domain-wide delegation');
}
