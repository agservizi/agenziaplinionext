...
try {
    ...
    if ($response['status'] < 200 || $response['status'] >= 300) {
        booking_json([
            'message' => booking_google_error_message($response, 'Errore nel recupero disponibilità'),
        ], 500);
    }
    ...
    booking_json([
        'message' => $error->getMessage() !== '' ? $error->getMessage() : 'Errore nel recupero disponibilità',
    ], 500);
}
