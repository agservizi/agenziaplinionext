...
async function saveAttachments(files: File[], ticketIdHint: string) {
  ...
  for (const [index, file] of files.entries()) {
    ...
    if (!ALLOWED_EXTENSIONS.has(extension.toLowerCase())) {
      console.warn(`[Admin Ticket Upload] Skipping file with disallowed extension: ${file.name}`);
      continue;
    }

    if (file.size > MAX_FILE_SIZE) {
      console.warn(`[Admin Ticket Upload] Skipping file exceeding size limit: ${file.name} (${file.size} bytes)`);
      continue;
    }
