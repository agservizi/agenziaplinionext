...
try {
    ...
    try {
        ...
        $message = trim($providerError->getMessage()) !== '' ? trim($providerError->getMessage()) : 'Errore provider non specificato.';
        if (!client_area_is_manual_visura_fallback_error($message)) {
            throw $providerError;
        }
        ...
        $providerResult = [
            ...
            'providerService' => $resolvedServiceLabel !== '' ? $resolvedServiceLabel : $serviceType,
            'status' => 'awaiting_manual_fulfillment',
            'providerRequestId' => '',
            'message' => 'Pagamento confermato. La pratica e stata presa in carico manualmente: il documento verra caricato nello storico appena evaso dal backoffice.',
            'documentUrl' => '',
            'documentBase64' => '',
            'summary' => [
    ...
    if (client_area_has_database_config()) {
        ...
        if ($manualFulfillment && $requestId > 0) {
            $magicLink = client_area_create_visure_magic_link($requestId);
            client_area_send_visure_magic_link_email([
                'requestId' => $requestId,
                'customerName' => $customerName,
                'customerEmail' => $email,
                'customerPhone' => $phone,
                'serviceLabel' => (string) ($providerResult['providerService'] ?? ($resolvedServiceLabel !== '' ? $resolvedServiceLabel : $serviceType)),
                'amountCents' => (int) ($stripeSession['amountTotal'] ?? 0),
                'currency' => (string) ($stripeSession['currency'] ?? 'eur'),
                'fallbackReason' => $fallbackReason,
                'magicLink' => (string) ($magicLink['link'] ?? ''),
                'expiresAt' => (string) ($magicLink['expiresAt'] ?? ''),
            ]);
        }
