...
function client_area_ensure_client_area_visure_magic_links_table(): void
{
    $db = client_area_require_db();
...
function client_area_visure_magic_link_secret(): string
{
    return trim((string) (public_api_env('VISURE_MAGIC_LINK_SECRET', public_api_env('CAF_PATRONATO_MAGIC_LINK_SECRET', public_api_env('ADMIN_PORTAL_SESSION_SECRET', public_api_env('CLIENT_PORTAL_SESSION_SECRET', 'ag-visure-magic-link')))) ?: 'ag-visure-magic-link'));
}

function client_area_visure_magic_link_ttl_hours(): int
{
    return max(24, (int) (public_api_env('VISURE_MAGIC_LINK_TTL_HOURS', '240') ?: '240'));
}

function client_area_visure_operator_email(): string
{
    return trim((string) (public_api_env('VISURE_OPERATOR_EMAIL', public_api_env('RESEND_TO', 'ag.servizi16@gmail.com')) ?: 'ag.servizi16@gmail.com'));
}

function client_area_hash_visure_magic_token(string $token): string
{
    return hash('sha256', client_area_visure_magic_link_secret() . ':' . $token);
}

function client_area_build_visure_magic_link(string $token): string
{
    $base = trim((string) (public_api_env('NEXT_PUBLIC_SITE_URL', 'https://agenziaplinio.it') ?: 'https://agenziaplinio.it'));
    $base = rtrim($base, '/');
    return $base . '/area-admin/visure?magic_token=' . rawurlencode($token);
}

function client_area_create_visure_magic_link(int $requestId): array
{
    client_area_ensure_client_area_visure_magic_links_table();

    $rawToken = bin2hex(random_bytes(32));
    $tokenHash = client_area_hash_visure_magic_token($rawToken);
    $expiresAt = new DateTimeImmutable('+' . client_area_visure_magic_link_ttl_hours() . ' hours');

    $db = client_area_require_db();
    $stmt = $db->prepare('INSERT INTO client_area_visure_magic_links (request_id, token_hash, expires_at) VALUES (?, ?, ?)');
    if (!$stmt) {
        throw new RuntimeException('Impossibile generare il magic link visure.');
    }

    $expiresAtValue = $expiresAt->format('Y-m-d H:i:s');
    $stmt->bind_param('iss', $requestId, $tokenHash, $expiresAtValue);
    $stmt->execute();
    $stmt->close();

    return [
        'rawToken' => $rawToken,
        'link' => client_area_build_visure_magic_link($rawToken),
        'expiresAt' => $expiresAt,
    ];
}

function client_area_is_manual_visura_fallback_error(string $message): bool
{
    $normalized = strtolower(trim($message));
    return str_contains($normalized, 'credito')
        || str_contains($normalized, 'credit')
        || str_contains($normalized, 'saldo')
        || str_contains($normalized, 'token insuff')
        || str_contains($normalized, 'insufficient')
        || str_contains($normalized, 'fondi');
}

function client_area_send_visure_magic_link_email(array $input): array
{
    $apiKey = trim((string) (public_api_env('RESEND_API_KEY', '') ?: ''));
    $from = trim((string) (public_api_env('RESEND_FROM', '') ?: ''));
    $to = client_area_visure_operator_email();
    ...
    $serviceLabel = trim((string) ($input['serviceLabel'] ?? 'Visura'));
    $amountCents = (int) ($input['amountCents'] ?? 0);
    $currency = strtoupper(trim((string) ($input['currency'] ?? 'EUR')));
    $fallbackReason = trim((string) ($input['fallbackReason'] ?? 'Credito provider insufficiente.'));
    $magicLink = trim((string) ($input['magicLink'] ?? ''));
    $expiresAt = $input['expiresAt'] instanceof DateTimeInterface
        ? $input['expiresAt']
        : new DateTimeImmutable('+' . client_area_visure_magic_link_ttl_hours() . ' hours');

    $expiresLabel = $expiresAt->format('d/m/Y H:i');
    $totalLabel = number_format($amountCents / 100, 2, ',', '.') . ' ' . $currency;
    $subject = 'Visura in evasione manuale #' . $requestId;
    ...
    $body = [
        ...
        'subject' => $subject,
        'text' => implode("\n", [
            'Richiesta visura #' . $requestId,
            'Cliente: ' . ($customerName !== '' ? $customerName : 'n/d'),
            'Email: ' . ($customerEmail !== '' ? $customerEmail : 'n/d'),
            'Telefono: ' . ($customerPhone !== '' ? $customerPhone : 'Non indicato'),
            'Servizio: ' . ($serviceLabel !== '' ? $serviceLabel : 'Visura'),
            'Importo pagato: ' . $totalLabel,
            'Motivo fallback: ' . $fallbackReason,
            'Scadenza link: ' . $expiresLabel,
            '',
            'Apri pratica: ' . $magicLink,
        ]),
        'html' => '<div style="background:#eef4f8;padding:28px;font-family:Helvetica,Arial,sans-serif;color:#0f172a;">'
            ...
            . '<table style="width:100%;border-collapse:collapse;font-size:14px;">'
            . '<tr><td style="padding:6px 0;color:#64748b;width:180px;">Pratica</td><td style="padding:6px 0;font-weight:700;color:#0f172a;">#' . $requestId . '</td></tr>'
            . '<tr><td style="padding:6px 0;color:#64748b;">Cliente</td><td style="padding:6px 0;font-weight:700;color:#0f172a;">' . htmlspecialchars($customerName !== '' ? $customerName : 'n/d', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</td></tr>'
            . '<tr><td style="padding:6px 0;color:#64748b;">Email</td><td style="padding:6px 0;font-weight:700;color:#0f172a;">' . htmlspecialchars($customerEmail !== '' ? $customerEmail : 'n/d', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</td></tr>'
            . '<tr><td style="padding:6px 0;color:#64748b;">Telefono</td><td style="padding:6px 0;font-weight:700;color:#0f172a;">' . htmlspecialchars($customerPhone !== '' ? $customerPhone : 'Non indicato', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</td></tr>'
            . '<tr><td style="padding:6px 0;color:#64748b;">Servizio</td><td style="padding:6px 0;font-weight:700;color:#0f172a;">' . htmlspecialchars($serviceLabel !== '' ? $serviceLabel : 'Visura', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</td></tr>'
            . '<tr><td style="padding:6px 0;color:#64748b;">Importo pagato</td><td style="padding:6px 0;font-weight:700;color:#0f172a;">' . htmlspecialchars($totalLabel, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</td></tr>'
            . '</table></div>'
            ...
            . '<p style="margin:8px 0 0;font-size:14px;line-height:1.6;color:#7c2d12;">' . htmlspecialchars($fallbackReason, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</p>'
            . '</div>'
            . '<div style="margin-top:24px;"><a href="' . htmlspecialchars($magicLink, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '" style="display:inline-block;background:#0f172a;color:#ffffff;text-decoration:none;padding:14px 22px;border-radius:999px;font-size:14px;font-weight:700;">Apri pratica con magic link</a></div>'
            . '<p style="margin:14px 0 0;font-size:13px;line-height:1.6;color:#475569;">Il link resta valido fino al <strong>' . htmlspecialchars($expiresLabel, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</strong>. Aprendo la pratica puoi completare l\'evasione manuale e allegare il documento da rendere disponibile al cliente.</p>'
            . '</div></div></div>',
    ];
    ...
    $status = (int) ($response['status'] ?? 0);
    return ['sent' => $status >= 200 && $status < 300, 'reason' => $status >= 200 && $status < 300 ? 'sent' : 'http_error'];
}
